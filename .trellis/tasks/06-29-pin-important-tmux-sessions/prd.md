...
- [ ] Pin and unpin actions are available from session context menus.
- [ ] Pinned sessions are visibly marked in the Dashboard.
- [ ] Pinned running sessions appear before non-pinned recents in the menu-bar popover.
- [ ] Pinned session names persist across app restart.
- [ ] Unknown or deleted pinned names are ignored in live session lists.
- [ ] A recreated session with a pinned name is prioritized again.
- [ ] Tests cover ordering, persistence loading, unknown pinned names, and recreated pinned names at the shared state/model seam.
