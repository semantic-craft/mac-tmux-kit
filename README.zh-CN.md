...
<!-- 把精选截图放进 assets/screenshots/ 后取消注释。Dashboard 请用一个临时(scratch)会话，
     以免把真实会话名/路径带进公开仓库。 -->
<!--
<p align="center">
...
|---|---|---|---|
| 它是什么 | 叠在 tmux **之上**的小型 GUI | 一个完整的终端替代品 | 一个 tmux 的**替代**多路复用器 |
| 保留你的 tmux | ✅ 是 | — | ❌ 取而代之 |
| 占用 | 菜单栏常驻、原生 | 较重（Electron 量级） | 轻，但自带运行时 |
| 账号 / 登录 | 无 | 需要 | 无 |
| 动你的配置 | **不装插件、不改 `~/.tmux.conf`** | 不适用 | 新的配置格式 |

想要终端里内置 AI？用 Warp。想要现代多路复用器、又不在意 tmux 在 SSH 场景的普及度？用 Zellij。想**继续用 tmux**、只是让它在 macOS 上顺手？**Tmux Kit。**
...
<p align="center">
  为喜欢把自己环境收拾得<em>干净</em>的人而做。如果那是你，点个 ⭐ 吧。
</p>
